...
export default class BaseModel {
  ...
  static queryRaw({options = {}, tableName}) {
    let joinStatement = options.join ? options.join : ''
    ...
    const query = `SELECT ${options.select || '*'} FROM ${table} ${joinStatement} ${whereStatement} ${groupStatement} ${sortStatement} ${limitStatement};`
  ...
  static queryStringRaw(query, params) {
    const databaseLayer = new DatabaseLayer(this.database, this.tableName)
    return databaseLayer.executeSql(query, params).then(res => {
      return res.rows.length ? res.rows : []
    })
  }
  ...
  static pullAll({api, method = 'getAll', params = {}, queryOptions = {page: 1, limit: 15, order: 'id'}}) {
    ...
    return api[toCamelCase(this.tableName)][method]({params}).then(res => {
      const data = res.data.data.map(item => {
        item['synced_at'] = moment().format('YYYY-MM-DD HH:mm:ss')
        return item
      })
  ...
  static getQueue() {
    ...
    return databaseLayer.executeSql(`SELECT *
                                     FROM ${this.tableName}
                                     WHERE (synced_at IS NULL OR updated_at > synced_at OR deleted_at IS NOT NULL);`)
  }
  ...
  push({api, method, payload}) {
    ...
    return api[toCamelCase(self.tableName)][method](payload).then(res => {
      ...
      if (err.response && (err.response.status === 404 || err.response.status === 500 || err.response.status === 422)) {
  ...
  _isUpdated() {
    return moment(this.updated_at).isAfter(this.synced_at)
  }
  ...
  static isUpdated({item}) {
    return moment(item.updated_at).isAfter(item.synced_at)
  }
